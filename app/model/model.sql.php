...
class intelisis extends sqlbase {
	...
	function iCambioPres($base, $nuevo, $cant, $part, $idord, $movID){
		//// Buscamos si hay un documento abierto, si existe introducimos las partidas en el, si no existe, creamos uno nuevo.
			...
			if($row['val']>0){
				$renglon=$row['renglon'];
				$this->query="UPDATE INVD SET CANTIDAD = CANTIDAD + $cant, CANTIDADINVENTARIO = CANTIDADINVENTARIO + $cant where id = $id and ARTICULO = '$nuevo' and ArticuloDestino = '$base' and renglon = $renglon";
	...
	function valInt($regWms){
		...
		foreach($regWms['cabecera'] as $cbc){
			...
			$this->query="SELECT * FROM CteEnviarA where cliente = '$cliente' and id = $enviarA";
			$res=$this->Ejecutaquerysimple();
			$row=sqlsrv_fetch_array($res);
			$suc = $row['ID'];
